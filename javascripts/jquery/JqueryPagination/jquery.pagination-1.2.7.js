...
!function ($) {
    ...
    var Page = function (element, options) {
        ...
        var eventHandler = function (event) {
            ...
            if (event.type === 'click' && $target.data('pageIndex') !== undefined && !$target.parent().hasClass('active')) {
                ...
                that._updateTotal(that.total);//每页显示数据条数更新后同步更新页数
    ...
    Page.prototype = {
        ...
        renderPagination: function (pageIndex) {
            ...
            this.$info.text(renderInfo(this.currentPageIndex, this.currentPageSize, this.total, this.options.infoFormat, this.pageCount));
            if (this.pageCount > 1) {
                this.$page.show();
    ...
    var renderInfo = function (currentPageIndex, currentPageSize, total, infoFormat, pageCount) {
        ...
        return infoFormat.replace('{start}', startNum).replace('{end}', endNum).replace('{total}', total)
            .replace('{currentPageIndex}', currentPageIndex + 1).replace('{pageCount}', pageCount);
    }
